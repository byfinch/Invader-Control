...
try {
    ...
    function status_badge(?string $status, string $err = ''): string {
        $status = $status ?: '-';
        $attr = $err !== '' ? ' data-err="' . h($err) . '" tabindex="0"' : '';
        return '<span class="status ' . status_class($status) . '"' . $attr . '><i></i>' . h(status_label($status)) . '</span>';
    }
    ...
    if (isset($_GET['hist'])) {
        ...
        try {
            ...
            foreach ($histRows as $r) {
                ...
                $plain = [];
                if (($r['note'] ?? '') !== '') $plain[] = 'bot: ' . $r['note'];
                if (($r['unote'] ?? '') !== '') $plain[] = 'kullanıcı: ' . $r['unote'];
                $out[] = [
                    'ts' => local_time($r['ts']),
                    'site' => (string) $r['site'],
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['act'] ?? '') === 'start_run') {
        ...
        $filterUrl = '';
        if (!empty($_POST['ukey'])) {
            $uk = (string) $_POST['ukey'];
            foreach (($CFG['sites'] ?? []) as $s) {
                if (gb_url_key($s['url'] ?? '') === $uk) { $filterUrl = (string) $s['url']; break; }
            }
            if ($filterUrl === '') { http_response_code(404); echo json_encode(['ok' => false, 'error' => 'Site bulunamadı. Liste değişmiş olabilir — sayfayı yenileyin.']); exit; }
        }
        $active = @json_decode((string) @file_get_contents("$BASE/data/jobs/active.json"), true);
        if (is_array($active) && in_array($active['status'] ?? '', ['queued', 'running'], true)) {
        ...
        if ($filterUrl !== '') $job['site_filter'] = $filterUrl;
        save_job($job);
        @file_put_contents("$BASE/data/jobs/active.json", json_encode(['id' => $id, 'status' => 'queued']), LOCK_EX);
    ...
    $states = []; $last = []; $history = []; $historyTotal = 0; $historyPages = 1; $lastRun = null;
    try {
        gb_init_db($DB_FILE);
...
/* sabit sütun düzeni: tablo kabı aşmasın, yatay scroll olmasın */
.monitor-table{table-layout:fixed}
.monitor-table th:nth-child(1){width:15%}.monitor-table th:nth-child(2){width:10%}.monitor-table th:nth-child(3){width:8%}.monitor-table th:nth-child(4){width:8%}.monitor-table th:nth-child(5){width:22%}.monitor-table th:nth-child(6){width:12%}.monitor-table th:nth-child(7){width:5%}.monitor-table th:nth-child(8){width:20%}
.monitor-table td{padding:14px 10px}
.monitor-table .site-url{max-width:100%}
.monitor-table .expect,.monitor-table .alt-expected,.monitor-table .alt-observed{display:block;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.row-actions{gap:6px;flex-wrap:nowrap}
.row-actions .btn{padding:4px 7px;font-size:10.5px}
.history-table .site-table{table-layout:fixed}
.history-table th:nth-child(1){width:14%}.history-table th:nth-child(2){width:20%}.history-table th:nth-child(3){width:8%}.history-table th:nth-child(4){width:8%}.history-table th:nth-child(5){width:5%}.history-table th:nth-child(6){width:13%}.history-table th:nth-child(7){width:32%}
.history-table .site-url{max-width:100%}
.history-table .ell{display:block;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-family:var(--head-font);font-size:11px;color:#7fae8d}
.history-table .note{max-width:none}
...
(function(){
  ...
  async function startRun(ukey){
    ...
    try{
      ...
      fd.append('act','start_run');
      if(ukey) fd.append('ukey', ukey);
      const response=await fetch(location.pathname,{method:'POST',body:fd,headers:{'X-Requested-With':'XMLHttpRequest'}});
...
(function(){
  ...
  if(siteRows){
    ...
    siteRows.addEventListener('click',function(e){
      ...
      if(chk){ if(window.startRun) window.startRun(chk.dataset.ukey||''); return; }
  ...
  async function go(page){
    try{
      ...
      tbody.innerHTML=d.rows.map(function(row){
        return '<tr><td class="time">'+esc(row.ts)+'</td><td><span class="site-url">'+esc(row.site)+'</span></td><td>'+row.status+'</td><td>'+row.ustatus+'</td><td class="http">'+esc(row.http)+'</td><td class="expect"><span class="ell" title="'+esc(row.alt)+'">'+esc(row.alt)+'</span></td><td class="note" title="'+esc(row.note_title||'')+'"><div class="clamp">'+(row.note_html||'-')+'</div></td></tr>';
...
<script>
/* hata/detay tooltip'i — delegasyon: tablo AJAX ile yenilense de çalışır */
(function(){
  ...
  function show(badge){
    ...
    tipText.textContent=badge.dataset.err;
  ...
  function scheduleHide(){ hideTimer=setTimeout(function(){ tip.classList.remove('show'); current=null; },180); }
  function cancelHide(){ if(hideTimer){clearTimeout(hideTimer);hideTimer=null;} }
  function inTip(el){ return el && el.closest && el.closest('#err-tip'); }
  document.addEventListener('mouseover',function(e){
    const b=e.target.closest && e.target.closest('.status[data-err]');
    if(b){ cancelHide(); if(current!==b) show(b); }
  });
  document.addEventListener('mouseout',function(e){
    const b=e.target.closest && e.target.closest('.status[data-err]');
    if(b && !b.contains(e.relatedTarget) && !inTip(e.relatedTarget)) scheduleHide();
  });
  /* dokunmatik: dokununca aç */
  document.addEventListener('click',function(e){
    const b=e.target.closest && e.target.closest('.status[data-err]');
    if(b && current!==b){ cancelHide(); show(b); }
  });
  document.addEventListener('focusin',function(e){
    const b=e.target.closest && e.target.closest('.status[data-err]');
    if(b){ cancelHide(); show(b); }
  });
  document.addEventListener('focusout',function(e){
    const b=e.target.closest && e.target.closest('.status[data-err]');
    if(b && !(e.relatedTarget && tip.contains(e.relatedTarget))) scheduleHide();
  });
  tip.addEventListener('mouseenter',cancelHide);
  tip.addEventListener('mouseleave',scheduleHide);
  window.addEventListener('scroll',function(){ if(current) show(current); },true);
  tip.querySelector('.tip-copy').addEventListener('click',async function(){
    const btn=this;
    try{ await navigator.clipboard.writeText(tipText.textContent); }
    catch(e){ const ta=document.createElement('textarea'); ta.value=tipText.textContent; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); ta.remove(); }
    btn.textContent='✓ kopyalandı'; btn.classList.add('done');
    setTimeout(function(){ btn.textContent='⧉ kopyala'; btn.classList.remove('done'); },1200);
  });
})();
</script>
</body></html>
