...
.history-table .expect{display:block;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.history-table .note{max-width:none;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
  ...
  async function go(page){
    try{
      ...
      tbody.innerHTML=d.rows.map(function(row){
        return '<tr><td class="time">'+esc(row.ts)+'</td><td><span class="site-url">'+esc(row.site)+'</span></td><td>'+row.status+'</td><td>'+row.ustatus+'</td><td class="http">'+esc(row.http)+'</td><td class="expect">'+esc(row.alt)+'</td><td class="note" title="'+esc(row.note_title||'')+'">'+(row.note_html||'-')+'</td></tr>';
